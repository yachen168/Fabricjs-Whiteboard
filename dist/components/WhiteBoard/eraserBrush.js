...
(function () {
  var _setBgOverlayColor = fabric.StaticCanvas.prototype.__setBgOverlayColor;
  ...
  fabric.util.object.extend(fabric.Object.prototype, {
     ...
     */
    toObject: function toObject(additionalProperties) {
  ...
  /** @lends fabric.EraserBrush.prototype */
  {
     ...
     * @param {fabric.Object} object
     */
    restoreObjectVisibility: function restoreObjectVisibility(object) {
      if (object && object._originalOpacity) {
        object.set({
          opacity: object._originalOpacity
        });
     ...
     */
    prepareCanvasBackgroundForLayer: function prepareCanvasBackgroundForLayer(layer) {
      if (layer === "overlay") {
        return;
      }
     ...
     */
    prepareCanvasOverlayForLayer: function prepareCanvasOverlayForLayer(layer) {
      var canvas = this.canvas;
     ...
     * @private
     */
    restoreCanvasDrawables: function restoreCanvasDrawables() {
      var canvas = this.canvas;
     ...
     * @param {'bottom' | 'top' | 'overlay'} layer
     */
    prepareCanvasObjectsForLayer: function prepareCanvasObjectsForLayer(layer) {
     ...
     * @param {'bottom' | 'top' | 'overlay'} layer
     */
    restoreCanvasObjectsFromLayer: function restoreCanvasObjectsFromLayer(layer) {
      if (layer !== "bottom") return;
     ...
     * @returns boolean render overlay above brush
     */
    prepareCanvasForLayer: function prepareCanvasForLayer(layer) {
     ...
     * @param {'bottom' | 'top' | 'overlay'} layer
     */
    restoreCanvasFromLayer: function restoreCanvasFromLayer(layer) {
     ...
     */
    renderBottomLayer: function renderBottomLayer() {
      var canvas = this.canvas;
      this.prepareCanvasForLayer("bottom");
      canvas.renderCanvas(canvas.getContext(), canvas.getObjects().filter(function (obj) {
     ...
     * 2. Render the brush
     */
    renderTopLayer: function renderTopLayer() {
      var canvas = this.canvas;
     ...
     */
    applyEraserToCanvas: function applyEraserToCanvas(path) {
      var canvas = this.canvas;
